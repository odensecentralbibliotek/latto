<!-- # Lists -->

<!-- : LIST (default) -->
<ul class="list">
  <li class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <div class="image-container">
      <a href="#"><img alt="Et auto-skaleret billede" src="http://lorempixel.com/800/200/city/" /></a>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </li>
  <li class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </li>
  <li class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <div class="image-container">
      <a href="#"><img alt="Et auto-skaleret billede" src="http://lorempixel.com/800/200/city/" /></a>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </li>
</ul>

<!-- : LIST UL LI TAG (floated) -->
<ul class="list floated">
  <li class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <div class="image-container">
      <a href="#">
        <img alt="Et auto-skaleret billede" src="http://lorempixel.com/100/100/city/" />
      </a>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </li>
  <li class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </li>
  <li class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <div class="image-container">
      <a href="#">
        <img alt="Et auto-skaleret billede" src="http://lorempixel.com/100/100/city/" />
      </a>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </li>
</ul>

<!-- : LIST HIGHLIGHTED UL LI TAG (floated highlighted) -->
<div> <!-- NOTE: This wrapper DIV is not required - This is only here to show the entire mark up. -->
  <ul class="list highlighted">
    <li class="list-item">
      <h3 class="heading">
        <a href="#">Lorum Ipsum</a>
      </h3>
      <div class="image-container">
        <a href="#"><img alt="Et auto-skaleret billede" src="http://lorempixel.com/800/200/city/" /></a>
      </div>
      <p class="content">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit...
      </p>
      <a href="#">Read more</a>
    </li>
  </ul>
  <ul class="list floated highlighted">
    <li class="list-item">
      <h3 class="heading">
        <a href="#">Lorum Ipsum</a>
      </h3>
      <div class="image-container">
        <a href="#">
          <img alt="Et auto-skaleret billede" src="http://lorempixel.com/100/100/city/" />
        </a>
      </div>
      <div class="content">
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit...</p>
        <a href="#">Read more</a>
      </div>
    </li>
    <li class="list-item">
      <h3 class="heading">
        <a href="#">Lorum Ipsum</a>
      </h3>
      <div class="image-container">
        <a href="#">
          <img alt="Et auto-skaleret billede" src="http://lorempixel.com/100/100/city/" />
        </a>
      </div>
      <div class="content">
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit...</p>
        <a href="#">Read more</a>
      </div>
    </li>
    <li class="list-item">
      <h3 class="heading">
        <a href="#">Lorum Ipsum</a>
      </h3>
      <div class="content">
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit...</p>
        <a href="#">Read more</a>
      </div>
    </li>
    <li class="list-item">
      <h3 class="heading">
        <a href="#">Lorum Ipsum</a>
      </h3>
      <div class="image-container">
        <a href="#">
          <img alt="Et auto-skaleret billede" src="http://lorempixel.com/100/100/city/" />
        </a>
      </div>
      <div class="content">
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit...</p>
        <a href="#">Read more</a>
      </div>
    </li>
  </ul>
</div>

<!-- : LIST UL NO ICONS (date-list) -->
<ul class="list date-list">
  <li class="list-item">
    <h3 class="heading"><a href="#">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</a></h3>
    <div class="date-wrapper">
      <div class="day">Onsdag</div>
      <div class="date-wrapper-inner">
	      <div class="date">8</div>
        <div class="month">AUG</div>
      </div>
    </div>
    <div class="content">      
      <p>Sted: Biblioteket</p>
      <p>Tidspunkt: 10.00 - 11.00</p>
      <p>Pris: Kr. 20,-</p>
    </div>
  </li>
  <li class="list-item">
    <h3 class="heading"><a href="#">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</a></h3>
    <div class="date-wrapper">
      <div class="day">Onsdag</div>
      <div class="date-wrapper-inner">
	      <div class="date">8</div>
        <div class="month">AUG</div>
      </div>
    </div>
    <div class="content">      
      <p>Sted: Biblioteket</p>
      <p>Tidspunkt: 10.00 - 11.00</p>
      <p>Pris: Kr. 20,-</p>
    </div>
  </li>
  <li class="list-item">
    <h3 class="heading"><a href="#">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</a></h3>
    <div class="date-wrapper">
      <div class="day">Onsdag</div>
      <div class="date-wrapper-inner">
	      <div class="date">8</div>
        <div class="month">AUG</div>
      </div>
    </div>
    <div class="content">      
      <p>Sted: Biblioteket</p>
      <p>Tidspunkt: 10.00 - 11.00</p>
      <p>Pris: Kr. 20,-</p>
    </div>
  </li>
  <li class="list-item last">
    <h3 class="heading"><a href="#">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</a></h3>
    <div class="date-wrapper">
      <div class="day">Onsdag</div>
      <div class="date-wrapper-inner">
	      <div class="date">8</div>
        <div class="month">AUG</div>
      </div>
    </div>
    <div class="content">      
      <p>Sted: Biblioteket</p>
      <p>Tidspunkt: 10.00 - 11.00</p>
      <p>Pris: Kr. 20,-</p>
    </div>
  </li>
</ul>

<!-- : LIST DIV WITH ICONS (date-list) -->
<div class="list date-list">
  <div class="list-item">
    <h3 class="heading"><a href="#">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</a></h3>
    <div class="date-wrapper">
      <div class="day">Onsdag</div>
      <div class="date-wrapper-inner">
	      <div class="date">8</div>
        <div class="month">AUG</div>
      </div>
    </div>
    <div class="content">      
      <p><i class="icon-home"></i> Biblioteket</p>
      <p><i class="icon-time"></i> 10.00 - 11.00</p>
      <p><i class="icon-credit-card"></i> Kr. 20,-</p>
    </div>
  </div>
  <div class="list-item">
    <h3 class="heading"><a href="#">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</a></h3>
    <div class="date-wrapper">
      <div class="day">Onsdag</div>
      <div class="date-wrapper-inner">
	      <div class="date">8</div>
        <div class="month">AUG</div>
      </div>
    </div>
    <div class="content">      
      <p><i class="icon-home"></i> Biblioteket</p>
      <p><i class="icon-time"></i> 10.00 - 11.00</p>
      <p><i class="icon-credit-card"></i> Kr. 20,-</p>
    </div>
  </div>
  <div class="list-item">
    <h3 class="heading"><a href="#">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</a></h3>
    <div class="date-wrapper">
      <div class="day">Onsdag</div>
      <div class="date-wrapper-inner">
	      <div class="date">8</div>
        <div class="month">AUG</div>
      </div>
    </div>
    <div class="content">      
      <p><i class="icon-home"></i> Biblioteket</p>
      <p><i class="icon-time"></i> 10.00 - 11.00</p>
      <p><i class="icon-credit-card"></i> Kr. 20,-</p>
    </div>
  </div>
  <div class="list-item last">
    <h3 class="heading"><a href="#">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</a></h3>
    <div class="date-wrapper">
      <div class="day">Onsdag</div>
      <div class="date-wrapper-inner">
	      <div class="date">8</div>
        <div class="month">AUG</div>
      </div>
    </div>
    <div class="content">      
      <p><i class="icon-home"></i> Biblioteket</p>
      <p><i class="icon-time"></i> 10.00 - 11.00</p>
      <p><i class="icon-credit-card"></i> Kr. 20,-</p>
    </div>
  </div>
</div>

<!-- : LIST DIV TAG (floated) -->
<div class="list floated">
  <div class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <div class="image-container">
      <a href="#">
        <img alt="Et auto-skaleret billede" src="http://lorempixel.com/100/100/city/" />
      </a>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </div>
  <div class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </div>
  <div class="list-item">
    <h3 class="heading">
      <a href="#">Lorum Ipsum</a>
    </h3>
    <div class="subheading">
      <span class="label label-info"><a href="#">Lorem</a></span>
      <span class="date">10. august 2012</span>
      <span class="author">af <a href="#" title="View user profile." class="username">Author name</a></span>
    </div>
    <div class="image-container">
      <a href="#">
        <img alt="Et auto-skaleret billede" src="http://lorempixel.com/100/100/city/" />
      </a>
    </div>
    <p class="content">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit...
    </p>
    <a href="#">Read more</a>
  </div>
</div>

<!-- : LIST (simple) -->
<ul class="list simple">
  <li class="list-item">
    <span class="date">10. aug</span>
    <h3 class="heading">
      <a href="#">Etiam Vehicula Porta Egestas</a>
    </h3>
  </li>
  <li class="list-item">
    <h3 class="heading">
      <a href="#">Vehicula Justo</a>
    </h3>
  </li>
  <li class="list-item">
    <h3 class="heading">
      <a href="#">Bibendum Sit Tellus Amet Fringilla</a>
    </h3>
  </li>
  <li class="list-item">
    <span class="date">8. jul</span>
    <h3 class="heading">
      <a href="#">Aenean lacinia bibendum nulla sed consectetur. Vestibulum id ligula porta felis euismod semper.</a>
    </h3>
  </li>
  <li class="list-item">
    <span class="date">22. sep</span>
    <h3 class="heading">
      <a href="#">Bibendum Sit Tellus Amet Fringilla</a>
    </h3>
  </li>
</ul>